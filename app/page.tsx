...
export default function PlanesCompleteBento() {
  ...
  const [currentProfile, setCurrentProfile] = useState('Mariana (Engenharia)');
  const [selectedFront, setSelectedFront] = useState('Todas as Frentes');
  ...
  const [lightboxPhoto, setLightboxPhoto] = useState<{ src: string; item: string; front: string; resp: string; data: string; gps: string } | null>(null);
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  // Dynamic Operational Metrics
  const [physicalProgress, setPhysicalProgress] = useState(61.8);
  const [financialSpent, setFinancialSpent] = useState(14.8);
  ...
  const handleApprove = (id: number) => {
    ...
    const nextProg = Number((physicalProgress + 0.6).toFixed(1));
    const nextSpent = Number((financialSpent + 0.15).toFixed(2));
    setPhysicalProgress(nextProg);
    setFinancialSpent(nextSpent);
    showNotification(`✓ Medição de ${item?.item || 'item'} aprovada! Avanço recalculado para ${nextProg}%.`);
  };

  const handleReject = (id: number) => {
    const item = validations.find(v => v.id === id);
    setValidations(validations.filter(v => v.id !== id));
    showNotification(`✕ Apontamento de ${item?.item || 'item'} devolvido para ajuste no campo.`);
  };

  const handleSaveFieldLog = () => {
    setShowLogModal(false);
    setValidations([
      {
        id: Date.now(),
        resp: currentProfile.includes('Carlos') ? 'Carlos Lima (Você)' : 'Mariana Alves (Eng.)',
        cargo: 'Encarregado de Campo',
        front: logFront,
        item: logActivity,
        qtd: `${logAmount} ${logUnit}`,
        data: 'Agora mesmo',
        gps: 'Japaratinga - Coordenada GPS verificada',
        status: 'Pendente',
        foto: 'https://images.unsplash.com/photo-1541888946425-d0fbb186156a?w=600&auto=format&fit=crop&q=80',
      },
      ...validations,
    ]);
    showNotification(`🚀 ${logAmount} ${logUnit} de ${logActivity} enviados para a Engenharia!`);
    setLogAmount(15);
    setHasPhoto(false);
  };
  ...
  // Month Chart Data
  const barChartData = [
    { month: 'Mai', plan: 54, real: 51, value: 'R$ 2,90M', deviation: '-3,0 p.p.' },
    { month: 'Jun', plan: 66, real: 62, value: 'R$ 3,85M', deviation: '-4,0 p.p.', date: 'Jun 2026' },
    { month: 'Jul', plan: 75, real: 71, value: 'R$ 4,10M', deviation: '-4,0 p.p.' },
    { month: 'Ago', plan: 84, real: 80, value: 'R$ 4,70M', deviation: '-4,0 p.p.' },
    { month: 'Set', plan: 92, real: 89, value: 'R$ 5,20M', deviation: '-3,0 p.p.' },
    { month: 'Out', plan: 97, real: 95, value: 'R$ 5,80M', deviation: '-2,0 p.p.' },
  ...
  return (
    <div className="min-h-screen outer-viewport p-3 md:p-6 lg:p-8 flex items-center justify-center antialiased font-sans">
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed top-6 right-6 z-50 bg-slate-950 text-white border border-slate-700 px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 text-xs font-semibold">
          <span className="w-2.5 h-2.5 rounded-full bg-[#d4ff00] animate-ping"></span>
          <span>{toastMessage}</span>
        </div>
      )}

      {/* Main Dashboard Frame */}
      <div className="dashboard-main-frame w-full max-w-[1380px] min-h-[840px] p-5 md:p-7 flex flex-col lg:flex-row gap-6 relative">
        
        {/* =========================================================================
            LEFT SIDEBAR
        ========================================================================= */}
        <aside className="w-full lg:w-64 flex flex-col justify-between shrink-0">
          <div>
            {/* Logo */}
            <div className="flex items-center gap-3 px-2 py-1 mb-7">
              <img src="data:image/png;base64," alt="Planes ENG" className="h-8 object-contain" />
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl text-[14px] transition text-left ${
                      isActive ? 'nav-pill-active font-semibold' : 'nav-pill-inactive font-medium'
                    }`}
                  >
          ...
          {/* Project Status Card */}
          <div className="white-bento-card p-5 mt-6 border border-slate-100 relative">
            ...
            <button
              ...
              className="w-full py-2.5 bg-slate-950 hover:bg-slate-800 text-white text-[12px] font-semibold rounded-full transition shadow-xs flex items-center justify-center gap-1.5"
            >
              <span>Exportar Status PDF</span>
              <span>↗</span>
            </button>
          </div>
        </aside>
        ...
        <div className="flex-1 flex flex-col gap-5">
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center flex-wrap gap-2.5">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#d4ff00] hover:bg-[#c4ed00] text-slate-950 font-bold text-xs rounded-full transition shadow-xs flex items-center gap-1.5"
              >
                <span>＋</span>
              ...
              {/* Profile Switcher */}
              <div className="flex items-center gap-2 bg-white border border-slate-200/80 rounded-full pl-1.5 pr-3 py-1 shadow-2xs">
                <div className="w-7 h-7 rounded-full bg-[#081e2b] text-white flex items-center justify-center font-bold text-xs shrink-0">
                  {currentProfile.charAt(0)}
                </div>
                <div className="text-left">
                  <span className="text-[9.5px] text-slate-400 block leading-tight font-medium">Perfil Ativo</span>
                  <select
                    value={currentProfile}
                    onChange={(e) => {
                      setCurrentProfile(e.target.value);
                      showNotification(`Perfil alternado para ${e.target.value}`);
                    }}
                    className="text-[11.5px] text-slate-900 font-bold bg-transparent border-none p-0 focus:outline-hidden cursor-pointer"
                  >
                    <option value="Mariana (Engenharia)">Mariana (Engenharia)</option>
                    <option value="Carlos (Campo)">Carlos (Campo)</option>
                    <option value="Ricardo (Diretoria)">Ricardo (Diretoria)</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="white-bento-card p-6 border border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-2">Incremento Rápido de Quantidade</label>
                    <div className="flex items-center gap-2">
                      {[5, 10, 25, 50].map((inc) => (
                        <button
                          key={inc}
                          onClick={() => setLogAmount(prev => prev + inc)}
                          className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 rounded-xl font-extrabold text-sm text-slate-900 transition"
                        >
                          +{inc}
                        </button>
                      ))}
                      <button
                        onClick={() => setLogAmount(0)}
                        className="px-3 py-2.5 text-xs text-rose-600 font-bold hover:bg-rose-50 rounded-xl"
                      >
                        Zerar
                      </button>
                    </div>
                ...
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200/70 flex flex-col justify-between">
                  <div>
                    <span className="text-xs text-slate-400 font-bold block uppercase tracking-wider mb-1">Total a Lançar</span>
                    <div className="text-4xl font-extrabold text-slate-950 my-1">
                      {logAmount} <span className="text-base font-bold text-slate-500">{logUnit}</span>
                    </div>
                    <span className="text-xs text-emerald-600 font-semibold">Localização GPS: Setor Norte (Auto-verificado)</span>
                  </div>

                  <div className="my-4">
                    <button
                      onClick={() => setHasPhoto(!hasPhoto)}
                      className={`w-full py-3 border-2 border-dashed rounded-2xl text-xs font-bold transition flex items-center justify-center gap-2 ${
                        hasPhoto ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-slate-300 text-slate-600 hover:bg-white'
                      }`}
                    >
                      <span>📷</span>
                      <span>{hasPhoto ? '✓ Foto de Evidência Anexada (IMG_8942.jpg)' : 'Clique para Anexar Foto do Canteiro'}</span>
                    </button>
                  </div>

                  <button
                    onClick={handleSaveFieldLog}
                    className="w-full py-3 bg-[#d4ff00] hover:bg-[#c4ed00] text-slate-950 font-extrabold text-sm rounded-full transition shadow-xs"
                  >
                    Salvar e Enviar para Engenharia 🚀
                  </button>
                </div>
              </div>
            </div>
          ...
          {activeNav === 'Fila de Validações' && (
            <div className="white-bento-card p-6 border border-slate-100">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Fila de Validações da Engenharia</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Apontamentos de campo aguardando aprovação técnica de medição</p>
                </div>
                <span className="bg-[#8eb3ff] text-slate-900 text-xs font-bold px-3 py-1 rounded-full">
                  {validations.length} Pendentes
                </span>
              </div>
              ...
              {validations.length === 0 ? (
                <div className="py-12 text-center text-slate-400">
                  ...
                  <strong className="text-slate-700 font-bold block">Tudo em dia!</strong>
                  <span className="text-xs">Todos os apontamentos de campo foram validados.</span>
                </div>
              ) : (
                <div className="divide-y divide-slate-100">
                  {validations.map((val) => (
                    <div key={val.id} className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-center gap-3.5">
                        <img
                          src={val.foto}
                          alt="Foto"
                        ...
                        <div>
                          ...
                          <span className="text-xs text-slate-400 block mt-0.5">
                            Frente: <strong className="text-slate-600">{val.front}</strong> · Por: {val.resp} ({val.cargo}) · {val.data}
                          </span>
                          <span className="text-[11px] text-slate-500 font-mono mt-0.5 block">📍 {val.gps}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 self-end md:self-auto">
                        <button
                          ...
                          className="px-4 py-2 border border-slate-200 text-xs font-semibold text-rose-600 hover:bg-rose-50 rounded-full transition"
                        >
                          Devolver
                        </button>
                        <button
                          onClick={() => handleApprove(val.id)}
                          className="px-5 py-2 bg-[#d4ff00] hover:bg-[#c4ed00] text-slate-950 text-xs font-extrabold rounded-full transition shadow-xs"
                        >
                          Aprovar Medição ✓
                        </button>
                      </div>
            ...
            <div className="white-bento-card p-6 border border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  ...
                  <span className="text-[11px] text-emerald-700 font-medium">78% de taxa de conclusão</span>
                </div>
                <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
                  <span className="text-xs font-bold text-amber-800 uppercase block">Pendências por Chuva</span>
                  <div className="text-2xl font-black text-amber-950 my-1">3</div>
                  <span className="text-[11px] text-amber-700 font-medium">Paralisação de terraplenagem</span>
                </div>
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <span className="text-xs font-bold text-slate-600 uppercase block">Atraso de Insumos</span>
                  <div className="text-2xl font-black text-slate-900 my-1">2</div>
                  <span className="text-[11px] text-slate-500 font-medium">Entrega de fôrmas metálicas</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeNav === 'Suprimentos' && (
            <div className="white-bento-card p-6 border border-slate-100">
              ...
              <p className="text-xs text-slate-400 mb-6">Controle de consumo, estoque e pedidos em trânsito</p>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200/60">
                  <span className="text-xs font-bold text-slate-500 uppercase block">Concreto Usinado FCK 30</span>
                  <div className="text-2xl font-black text-slate-900 my-1.5">1.240 m³</div>
                  <span className="text-xs text-slate-500 block mb-3">Consumido de 2.000 m³ contratados</span>
                  <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                    <div className="h-full bg-[#d4ff00] rounded-full" style={{ width: '62%' }}></div>
                  </div>
                </div>

                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200/60">
                  <span className="text-xs font-bold text-slate-500 uppercase block">Aço Estrutural CA-50</span>
                  <div className="text-2xl font-black text-slate-900 my-1.5">88,5 Ton</div>
                  <span className="text-xs text-slate-500 block mb-3">Consumido de 120 Ton previstas</span>
                  <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                    <div className="h-full bg-[#8eb3ff] rounded-full" style={{ width: '74%' }}></div>
                  </div>
                </div>

                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200/60">
                  <span className="text-xs font-bold text-slate-500 uppercase block">Argamassa e Revestimentos</span>
                  <div className="text-2xl font-black text-slate-900 my-1.5">4.200 Sacos</div>
                  <span className="text-xs text-slate-500 block mb-3">Estoque suficiente para 22 dias</span>
                  <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
          ...
          {activeNav === 'Relatórios' && (
            <div className="white-bento-card p-6 border border-slate-100">
              ...
              <p className="text-xs text-slate-400 mb-6">Emissão de relatórios diários de obra (RDO), Curva S e auditorias</p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200/60 flex items-center justify-between">
                  <div>
                    <strong className="text-sm font-bold text-slate-900 block">Relatório Diário de Obra (RDO)</strong>
                    <span className="text-xs text-slate-400">Gerado hoje · {workersCount} operários</span>
                  </div>
                  <button onClick={() => setShowExportModal(true)} className="px-4 py-2 bg-slate-900 text-white rounded-full text-xs font-bold">
                    Baixar PDF
                  </button>
                </div>
                ...
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200/60 flex items-center justify-between">
                  <div>
                    <strong className="text-sm font-bold text-slate-900 block">Consolidado Físico-Financeiro</strong>
                    <span className="text-xs text-slate-400">Curva S e Aderência PAC</span>
                  </div>
                  <button onClick={() => setShowExportModal(true)} className="px-4 py-2 bg-slate-900 text-white rounded-full text-xs font-bold">
                    Baixar Excel
                  </button>
      ...
      {/* Lightbox Modal */}
      {lightboxPhoto && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setLightboxPhoto(null)}>
          <div className="bg-white rounded-3xl overflow-hidden max-w-xl w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <img src={lightboxPhoto.src} alt="Foto de Campo" className="w-full h-72 object-cover" />
            <div className="p-6">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h4 className="font-bold text-base text-slate-900">{lightboxPhoto.item}</h4>
                  <span className="text-xs text-slate-500">Frente: <strong>{lightboxPhoto.front}</strong> · Por: {lightboxPhoto.resp}</span>
                </div>
                <span className="bg-[#d4ff00] text-slate-950 text-xs font-extrabold px-3 py-1 rounded-full">Auditado</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl text-xs text-slate-600 font-mono space-y-1 mb-4 border border-slate-100">
                <div>📍 GPS: {lightboxPhoto.gps}</div>
                <div>🕒 Timestamp: {lightboxPhoto.data}</div>
              </div>
              <button onClick={() => setLightboxPhoto(null)} className="w-full py-2.5 bg-slate-950 text-white text-xs font-bold rounded-full">
                Fechar Inspeção
              </button>
            </div>
          </div>
      ...
      {/* Universal New Log Modal */}
      {showLogModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-100">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
              <div className="flex items-center gap-2.5">
                <span className="w-8 h-8 rounded-full bg-[#d4ff00] flex items-center justify-center font-bold text-slate-900">⚡</span>
                <div>
                  <h3 className="text-base font-bold text-slate-900">Novo Lançamento de Campo</h3>
                  <span className="text-[11px] text-slate-400">Planes 1-Touch</span>
                </div>
              </div>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button
                onClick={handleSaveFieldLog}
                className="flex-1 py-2.5 rounded-full bg-[#d4ff00] text-slate-950 font-extrabold text-xs hover:bg-[#c4ed00] shadow-xs"
              >
                Salvar Lançamento
              </button>
            </div>
      ...
      {showExportModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-100 text-center">
            <img src="data:image/png;base64," alt="P" className="w-12 h-12 rounded-2xl mx-auto mb-3 shadow-xs object-contain" />
            <h3 className="text-base font-bold text-slate-900">Exportar Relatório Gerencial</h3>
            <p className="text-xs text-slate-500 my-2">Japaratinga Resort · Expansão Fase 3 · Curva S consolidada e avanço por frentes</p>
            <div className="my-4 p-3 bg-slate-50 rounded-2xl border border-slate-100 text-left text-xs space-y-1.5">
              <div className="flex justify-between"><span className="text-slate-500">Avanço Geral:</span><strong>{physicalProgress}%</strong></div>
              <div className="flex justify-between"><span className="text-slate-500">Aderência PAC:</span><strong>{pacScore}%</strong></div>
              <div className="flex justify-between"><span className="text-slate-500">Custo Medido:</span><strong>R$ {financialSpent}M</strong></div>
            </div>
            <div className="flex gap-2">
              <button
              ...
              <button
                onClick={() => { setShowExportModal(false); alert('Relatório Executivo gerado com sucesso!'); }}
                className="flex-1 py-2.5 rounded-full bg-slate-950 text-white text-xs font-bold hover:bg-slate-800 shadow-xs"
              >
                Baixar PDF
              </button>
            </div>
