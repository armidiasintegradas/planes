...
export default function Home() {
  ...
  const [selectedFront, setSelectedFront] = useState('Todas');
  ...
  const approvedCount = entries.filter(e => e.status === 'Aprovado').length;
  const baseProgress = 61.8 + (approvedCount * 0.4);
  const currentPhysicalProgress = Math.min(100, Number(baseProgress.toFixed(1)));
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row text-slate-800 antialiased selection:bg-[#38bdf8] selection:text-white">
      ...
      {/* Sidebar Navigation */}
      <aside className={`fixed md:sticky top-0 z-40 h-screen w-64 bg-[#081e2b] text-white flex flex-col shrink-0 border-r border-slate-800/80 transition-transform duration-300 ${mobileMenu ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        {/* Brand Header */}
        <div className="h-20 flex items-center justify-between px-6 border-b border-white/10 bg-[#04131d]">
          <div className="flex items-center gap-3">
            <img src={PLANES_ICON_B64} alt="Planes Icon" className="w-8 h-8 rounded-lg shadow-sm" />
            <div>
              <span className="font-bold text-white text-base tracking-wider block leading-none font-sans">PLANES</span>
        ...
        <div className="m-4 p-3.5 rounded-xl bg-white/5 border border-white/10">
          ...
          <span className="text-[10px] text-slate-400 block mt-0.5 font-mono">Expansão 3 · Alagoas</span>
        </div>
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          {availableNav.map(([id, label, icon]) => {
            ...
            const badge = id === 'validation' && pendingCount > 0 ? (
              <span className="ml-auto bg-[#f59e0b] text-[#081e2b] text-[10px] font-bold px-2 py-0.5 rounded-full">{pendingCount}</span>
            ) : null;
            return (
              <button
                key={id}
        ...
        <div className="p-4 border-t border-white/10 flex items-center justify-between bg-black/20">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 rounded-full bg-cyan-200 text-[#081e2b] font-bold text-xs flex items-center justify-center shrink-0">
              {user.avatar}
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#f8fafc]">
        {/* Modern Fluid Header */}
        <header className="h-16 glass-header px-4 md:px-8 flex items-center justify-between sticky top-0 z-20 shadow-xs">
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => setModalMode('Produção')}
              className="bg-[#0c6a91] hover:bg-[#081e2b] text-white text-xs font-semibold px-3 py-1.5 rounded-xl transition shadow-xs flex items-center gap-1.5"
            >
              <span>＋</span>
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-medium transition ${user.role === 'Engenharia' ? 'bg-white text-[#081e2b] shadow-xs font-semibold' : 'text-slate-600 hover:text-slate-900'}`}
              >
                Engenharia
              </button>
              <button
                onClick={() => switchRole('campo@planes.demo')}
                className={`px-2.5 py-1 rounded-lg font-medium transition ${user.role === 'Campo' ? 'bg-white text-[#081e2b] shadow-xs font-semibold' : 'text-slate-600 hover:text-slate-900'}`}
              >
                Campo
              </button>
              <button
                onClick={() => switchRole('diretoria@planes.demo')}
                className={`px-2.5 py-1 rounded-lg font-medium transition ${user.role === 'Diretoria' ? 'bg-white text-[#081e2b] shadow-xs font-semibold' : 'text-slate-600 hover:text-slate-900'}`}
              >
                Diretoria
              </button>
            </div>
        ...
        {/* Dynamic Views */}
        <main className="flex-1 p-4 md:p-8 max-w-6xl w-full mx-auto animate-fluid-entry">
          {view === 'dashboard' && (
            <DashboardFluidView
              entries={entries}
              setView={setView}
              ...
              physicalProgress={currentPhysicalProgress}
            />
          )}
          {view === 'field' && (
            <FieldFluidView
              onOpenModal={(mode, actId) => { setModalMode(mode); if (actId) setSelectedActivityId(actId); }}
            />
          )}
          {view === 'validation' && (
            <ValidationFluidView entries={entries} onDecide={handleDecideEntry} />
          )}
          {view === 'gantt' && <GanttFluidView />}
          {view === 'balance' && <BalanceFluidView />}
          {view === 'lookahead' && (
            <LookaheadFluidView
              weeks={lookaheadWeeks}
              setWeeks={setLookaheadWeeks}
              onPromote={() => notify('Atividade promovida para o plano semanal!')}
            />
          )}
          {view === 'weekly' && <WeeklyFluidView />}
          {view === 'constraints' && (
            <ConstraintsFluidView constraints={constraints} onNew={() => setModalMode('NovaRestricao')} />
          )}
          {view === 'estrutura' && <StructureFluidView />}
        </main>
      </div>

      {/* Entry Modal */}
      {modalMode && modalMode !== 'NovaRestricao' && (
        <div className="fixed inset-0 bg-[#081e2b]/75 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 relative shadow-2xl border border-slate-200 animate-fluid-entry">
            ...
            <h3 className="text-xl text-[#081e2b] font-bold mt-1 mb-4 font-editorial">Registrar {modalMode}</h3>
            ...
            <form onSubmit={handleCreateEntry} className="space-y-4">
              ...
              {modalMode !== 'Impedimento' && (
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-xs font-semibold text-slate-700">Quantidade executada</label>
                    <div className="flex gap-1">
                      {['+5', '+10', '+25'].map(p => (
                        <button
                          type="button"
                          key={p}
                          onClick={() => setEntryQty(String((Number(entryQty) || 0) + Number(p.replace('+', ''))))}
                  ...
                  <input type="number" step="0.1" required value={entryQty} onChange={e => setEntryQty(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-mono-numbers" />
                </div>
              )}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Registro Fotográfico</label>
                <div
                  onClick={() => setHasPhoto(!hasPhoto)}
                  className={`border-2 border-dashed rounded-xl p-3 text-center text-xs cursor-pointer transition ${
                    hasPhoto ? 'border-emerald-500 bg-emerald-50/50 text-emerald-800' : 'border-slate-300 hover:border-[#0c6a91] text-[#0c6a91]'
                  }`}
                >
                  {hasPhoto ? '✓ Foto anexada: IMG_2026_0821_CANTEIRO.JPG (2.4 MB)' : '📷 Clique para anexar registro fotográfico do canteiro'}
                </div>
              </div>

              <button type="submit" className="w-full py-3 bg-[#0c6a91] hover:bg-[#081e2b] text-white font-semibold text-xs rounded-xl transition shadow-sm mt-2">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 bg-[#081e2b] text-white px-5 py-3 rounded-xl shadow-2xl text-xs flex items-center gap-2 border border-white/10 animate-fluid-entry">
...
/* =========================================================================
   SUB-VIEWS: MODERN FLUID DESIGN & DATA STORYTELLING
========================================================================= */

function LoginView({ email, setEmail, password, setPassword, error, onLogin, onQuickDemo }: any) {
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc]">
      <div className="w-full md:w-1/2 bg-gradient-to-br from-[#04131d] via-[#081e2b] to-[#0d2a3c] text-white p-8 md:p-16 flex flex-col justify-between relative overflow-hidden">
        <div className="flex items-center gap-3">
        ...
        <div className="border-t border-white/15 pt-6 relative z-10">
          ...
          <span className="text-xs text-slate-400 block mt-0.5 font-mono">16 frentes monitoradas · Linha de balanço integrada</span>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 md:p-12 bg-white">
        <div className="w-full max-w-md">
          ...
          <h2 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 mb-2 font-editorial">Bem-vindo à sua obra</h2>
          ...
          <div className="mb-6 p-4 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <button onClick={() => onQuickDemo('engenharia@planes.demo')} className="p-2.5 rounded-xl bg-white border border-slate-300 hover:border-[#38bdf8] hover:bg-cyan-50 text-left transition text-xs shadow-2xs">
                <strong className="block text-[#081e2b]">Engenharia</strong>
              ...
              <button onClick={() => onQuickDemo('campo@planes.demo')} className="p-2.5 rounded-xl bg-white border border-slate-300 hover:border-[#38bdf8] hover:bg-cyan-50 text-left transition text-xs shadow-2xs">
                <strong className="block text-[#081e2b]">Campo</strong>
              ...
              <button onClick={() => onQuickDemo('diretoria@planes.demo')} className="p-2.5 rounded-xl bg-white border border-slate-300 hover:border-[#38bdf8] hover:bg-cyan-50 text-left transition text-xs shadow-2xs">
                <strong className="block text-[#081e2b]">Diretoria</strong>
          ...
          <form onSubmit={onLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full py-3 bg-[#0c6a91] hover:bg-[#081e2b] text-white font-semibold text-xs rounded-xl transition shadow-sm flex items-center justify-center gap-2">
...
function DashboardFluidView({ entries, setView, activePoint, setActivePoint, selectedFront, setSelectedFront, physicalProgress }: any) {
  const pendingCount = entries.filter((e: any) => e.status === 'Pendente').length;
  const adherenceCircumference = 2 * Math.PI * 40;
  const adherenceOffset = adherenceCircumference * (1 - 0.78);

  const fronts = ['Todas', 'Arena', 'Bloco de Apartamentos', 'Infraestrutura', 'Área da Piscina'];

  return (
    <div className="space-y-6">
      {/* Title & Front Filter Pills */}
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          <span className="eyebrow text-[#0c6a91] block">VISÃO EXECUTIVA · FLUID DATA STORYTELLING</span>
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Painel Planes</h1>
        </div>

        {/* Dynamic Front Pill Selector */}
        <div className="pill-nav-container flex items-center gap-1 overflow-x-auto self-start">
          {fronts.map(f => (
            <button
              key={f}
              onClick={() => setSelectedFront(f)}
              className={`pill-tab whitespace-nowrap ${selectedFront === f ? 'pill-tab-active' : 'pill-tab-inactive'}`}
            >
              {f}
            </button>
          ))}
      ...
      {/* Main Metric Banner */}
      <div className="glass-dark p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className="eyebrow text-slate-300">STATUS DA OBRA</span>
          <span className="bg-amber-400 text-[#04131d] text-[10px] font-bold px-2.5 py-0.5 rounded-full">● Atenção</span>
        </div>
        <p className="text-xs text-slate-200 m-0">
          Filtro ativo: <strong>{selectedFront}</strong>. Ritmo abaixo do planejado em <strong>Bloco de Apartamentos</strong> e <strong>Piscina</strong>.
        </p>
        <span className="text-[10px] text-slate-400 font-mono">Consolidação: 21/08 · 17:40</span>
      </div>

      {/* Fluid Bento Metric Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* Metric 1: Real-time Physical Progress */}
        <div className="glass-panel p-5">
          <div className="flex justify-between text-xs text-slate-500 font-medium">
            <span>Avanço Físico Ponderado</span>
            <span className="text-slate-400 font-mono text-[10px]">EAP</span>
          </div>
          <div className="display-stat text-3xl text-[#081e2b] my-2 font-mono-numbers">{physicalProgress}%</div>
          ...
          <div className="mt-3 pt-2 border-t border-slate-100 text-[11px] text-rose-600 font-semibold flex items-center justify-between">
            <span>{(physicalProgress - 66.4).toFixed(1)} p.p. de desvio</span>
            <span className="text-[10px] bg-rose-50 text-rose-700 px-1.5 py-0.5 rounded">Atenção</span>
          </div>
        </div>

        {/* Metric 2: Radial Gauge Adherence */}
        <div className="glass-panel p-5 flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-xs text-slate-500 font-medium block">Aderência Semanal (PAC)</span>
              <div className="display-stat text-3xl text-[#081e2b] mt-2 font-mono-numbers">78%</div>
        ...
        <div className="glass-panel p-5">
          ...
          <div className="display-stat text-3xl text-[#081e2b] my-2 font-mono-numbers">+18 dias</div>
        ...
        {/* Metric 4: Constraints & Pending Validations */}
        <div className="glass-panel p-5">
          <div className="flex justify-between text-xs text-slate-500 font-medium">
            <span>Restrições Abertas</span>
            <span className="text-slate-400 font-mono text-[10px]">LOOKAHEAD</span>
          </div>
          <div className="display-stat text-3xl text-[#081e2b] my-2 font-mono-numbers">12</div>
      ...
      {/* Interactive S-Curve Chart & Decision Column */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
            <div>
              ...
              <h3 className="text-lg text-[#081e2b] font-bold font-editorial">Curva S de Avanço Físico</h3>
            </div>
            
            {activePoint && (
              <div className="text-xs bg-[#081e2b] text-white px-3 py-1.5 rounded-lg font-mono-numbers flex items-center gap-2 shadow-xs">
                <span className="text-[#38bdf8] font-bold">{activePoint.month}/26:</span>
          ...
          <div className="h-56 relative mt-2">
            <svg viewBox="0 0 700 220" preserveAspectRatio="none" className="w-full h-48 overflow-visible">
              ...
              {/* Area under curve */}
              <path fill="url(#fluidGradient)" opacity="0.9" d="M 30 185 L 105 170 L 180 148 L 255 124 L 330 100 L 405 78 L 405 210 L 30 210 Z" />

              {/* Baseline Curve */}
              <path className="chart-baseline-curve" d="M 30 185 C 80 175, 130 155, 180 145 S 280 105, 330 92 S 430 55, 480 46 S 580 15, 630 5" />

              {/* Actual Curve */}
              <path className="chart-actual-curve" d="M 30 185 C 80 178, 130 158, 180 148 S 280 112, 330 100 S 370 85, 405 78" />

              {/* Scrubber vertical line for active month */}
              {activePoint && (
                <line x1={activePoint.x} y1={10} x2={activePoint.x} y2={205} className="scrubber-line" />
              )}
                ...
                <g key={pt.month} onClick={() => setActivePoint(pt)} className="cursor-pointer group">
                  ...
                  {pt.real !== null && (
                    <circle
                      cx={pt.x}
                      cy={pt.yReal!}
                      r={activePoint?.month === pt.month ? "7" : "4.5"}
                      fill="#38bdf8"
                      stroke="#ffffff"
                      strokeWidth={activePoint?.month === pt.month ? "3" : "2"}
            ...
            {/* X-Axis Month Chips */}
            <div className="flex justify-between text-[11px] text-slate-400 font-mono-numbers mt-1 px-4">
              {SCURVE_DATA.map(pt => (
                <button
                  key={pt.month}
                  onClick={() => setActivePoint(pt)}
                  className={`hover:text-slate-900 transition ${activePoint?.month === pt.month ? 'font-bold text-[#0c6a91] text-xs' : ''}`}
                >
                  {pt.month}
                </button>
              ))}
          ...
          <div className="flex items-center gap-6 mt-4 pt-3 border-t border-slate-100 text-xs text-slate-600">
            ...
            <span className="flex items-center gap-2"><span className="w-4 h-1 bg-[#38bdf8] rounded-full" /> Realizado Acumulado ({physicalProgress}% em Ago)</span>
          </div>
        </div>
        ...
        <div className="glass-panel p-6 flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-lg text-[#081e2b] font-bold font-editorial mb-3">Atenções para Decisão</h3>
      ...
      <div className="glass-panel p-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <h3 className="text-lg text-[#081e2b] font-bold font-editorial">Produção por Frente de Obra</h3>
          </div>
          ...
          {[
            ...
            <div key={f.name} className="p-4 rounded-xl bg-slate-50 border border-slate-200">
              <div className="flex justify-between items-center text-xs mb-2">
                <strong className="text-[#081e2b]">{f.name}</strong>
              ...
              <div className="flex justify-between text-[11px] text-slate-500 font-mono-numbers">
...
function FieldFluidView({ onOpenModal }: { onOpenModal: (mode: 'Produção' | 'Material' | 'Impedimento', actId?: string) => void }) {
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <span className="eyebrow text-[#0c6a91] block">CAMPO</span>
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Minha Obra</h1>
        </div>
      ...
      <div className="glass-dark p-6 md:p-8 mb-8">
      ...
      <div className="mb-4 flex justify-between items-center">
        <div>
          ...
          <h3 className="text-lg text-[#081e2b] font-bold font-editorial">Atividades em Foco (Semana 34)</h3>
        </div>
        <span className="bg-slate-200 text-slate-700 text-xs font-semibold px-2.5 py-1 rounded-lg font-mono-numbers">4 prioritárias</span>
      </div>

      <div className="space-y-3">
        {initialActivities.slice(4, 8).map(a => (
          <div key={a.id} className="glass-panel p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <span className={`w-2.5 h-2.5 rounded-full ${a.status === 'Atrasada' ? 'bg-rose-500' : a.status === 'Atenção' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
              <div>
                <strong className="text-sm text-[#081e2b] block font-semibold">{a.name}</strong>
            ...
            <div className="flex items-center gap-6">
              <div className="w-32">
                <div className="flex justify-between text-[10px] text-slate-500 font-semibold mb-1 font-mono-numbers">
                  <span>Avanço</span>
              ...
              <button onClick={() => onOpenModal('Produção', a.id)} className="px-3.5 py-1.5 rounded-xl bg-[#0c6a91] text-white text-xs font-semibold hover:bg-[#081e2b] transition">
                Apontar →
              </button>
            </div>
          </div>
...
function ValidationFluidView({ entries, onDecide }: { entries: Entry[]; onDecide: (id: string, dec: 'Aprovado' | 'Devolvido') => void }) {
    ...
    <div>
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Fila de Validações</h1>
        </div>
        <span className="bg-amber-100 text-amber-900 text-xs font-bold px-3 py-1.5 rounded-full font-mono-numbers">{pending.length} pendentes</span>
      </div>
      ...
      {pending.length === 0 ? (
        <div className="glass-panel p-16 text-center">
          ...
          <h3 className="text-xl text-[#081e2b] font-bold font-editorial">Fila de validação em dia!</h3>
          <p className="text-xs text-slate-500 mt-1">Todos os apontamentos submetidos pelo campo foram analisados e os indicadores consolidados.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {pending.map(e => {
            ...
            return (
              <div key={e.id} className="glass-panel p-5 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center text-xs mb-3">
                    <span className="bg-cyan-100 text-[#081e2b] font-bold px-2.5 py-1 rounded-md text-[10px] uppercase">{e.kind}</span>
                    <span className="text-slate-400 text-[10px] font-mono-numbers">{e.date}</span>
                  </div>
                  <h4 className="font-semibold text-sm text-[#081e2b]">{act.name}</h4>
                  ...
                  <div className="my-4 p-3.5 bg-slate-50 rounded-xl border border-slate-100">
                    <div className="text-lg text-[#0c6a91] font-bold font-mono-numbers">{e.kind === 'Produção' ? `${e.quantity} unidades executadas` : `Registro de ${e.kind}`}</div>
    ...
    <div>
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Cronograma · Baseline × Atual</h1>
        </div>
      ...
      <div className="glass-panel overflow-hidden">
        <div className="grid grid-cols-7 bg-slate-50 border-b border-slate-200 text-[10px] font-bold text-slate-600 uppercase tracking-wider p-3.5 font-mono-numbers">
          ...
          {initialActivities.slice(4, 12).map((a, i) => (
            <div key={a.id} className="grid grid-cols-7 p-3.5 items-center text-xs">
              <div className="col-span-3 pr-4">
                <strong className="text-xs text-[#081e2b] block font-semibold">{a.name}</strong>
              ...
              <div className="col-span-4 relative h-8 bg-slate-50/50 rounded-xl flex items-center px-2">
                ...
                <span className="absolute right-2 text-[10px] font-bold text-slate-600 font-mono-numbers">{a.progress}%</span>
              </div>
            </div>
          ))}
        </div>
    ...
    <div>
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Linha de Balanço</h1>
        </div>
      ...
      <div className="glass-panel p-6">
        <div className="grid grid-cols-6 text-center text-[10px] font-bold text-slate-500 uppercase pb-3 border-b border-slate-100 font-mono-numbers">
...
function LookaheadFluidView({ weeks, setWeeks, onPromote }: any) {
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-3">
        <div>
          <span className="eyebrow text-[#0c6a91] block">MÉDIO PRAZO</span>
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Lookahead ({weeks} Semanas)</h1>
        </div>
        <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl p-1 font-mono-numbers">
          {[2, 3, 4, 5, 6].map(w => (
            <button key={w} onClick={() => setWeeks(w)} className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition ${weeks === w ? 'bg-[#081e2b] text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
              {w} sem
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {['Semana 34 (17–21 ago)', 'Semana 35 (24–28 ago)', 'Semana 36 (31 ago–04 set)', 'Semana 37 (07–11 set)'].slice(0, weeks).map((w, idx) => (
          <div key={w} className="bg-slate-100 p-3.5 rounded-2xl border border-slate-200 flex flex-col gap-3">
            <div className="font-bold text-xs text-[#081e2b] border-b border-slate-200 pb-2 font-mono-numbers">{w}</div>
            {initialActivities.slice(idx * 2, idx * 2 + 2).map(a => (
              <div key={a.id} className="glass-panel p-3.5 shadow-2xs">
...
function WeeklyFluidView() {
  return (
    <div>
      <div className="flex justify-between items-end mb-6">
        <div>
          <span className="eyebrow text-[#0c6a91] block">COMPROMISSOS</span>
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Plano Semanal de Produção</h1>
        </div>
        <div className="text-xs bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-slate-600 font-medium font-mono-numbers">Semana 34 · 17 a 21 de Agosto</div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="glass-panel p-5">
          <span className="eyebrow text-slate-400 block">ADERÊNCIA</span>
          <div className="display-stat text-3xl text-[#081e2b] my-1 font-mono-numbers">78%</div>
        ...
        <div className="glass-panel p-5">
          ...
          <div className="display-stat text-3xl text-amber-600 my-1 font-mono-numbers">4</div>
        ...
        <div className="glass-panel p-5">
          ...
          <div className="display-stat text-3xl text-rose-600 my-1 font-mono-numbers">5</div>
          ...
          {initialActivities.slice(4, 10).map((a, i) => (
            <div key={a.id} className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 pr-4">
                <strong className="font-semibold text-[#081e2b] block">{a.name}</strong>
                <span className="text-[10px] text-slate-500">{a.location}</span>
              </div>
              <div className="text-slate-600">{i % 2 === 0 ? 'Carlos (Campo)' : 'Equipe Alfa'}</div>
              <div className="font-semibold text-slate-700 font-mono-numbers">{a.progress}% / 100%</div>
              <div>
...
function ConstraintsFluidView({ constraints, onNew }: any) {
  return (
    <div>
      <div className="flex justify-between items-end mb-6">
        <div>
          <span className="eyebrow text-[#0c6a91] block">GESTÃO DE IMPEDIMENTOS</span>
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Restrições da Obra</h1>
        </div>
        <button onClick={onNew} className="bg-[#0c6a91] text-white px-3.5 py-2 rounded-xl text-xs font-semibold hover:bg-[#081e2b] transition shadow-xs">＋ Nova Restrição</button>
      </div>
        ...
        <div className="divide-y divide-slate-100 text-xs">
          {constraints.map((c: any) => (
            <div key={c.id} className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 pr-4">
                <strong className="font-semibold text-[#081e2b] block">{c.title}</strong>
              ...
              <div>
                ...
                <div className="text-[10px] text-slate-400 font-mono-numbers">Prazo: {c.due}</div>
              </div>
              <div>
    ...
    <div>
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl text-[#081e2b] font-bold mt-1 font-editorial">Hierarquia da Obra</h1>
        </div>
        <div className="text-xs bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-slate-600 font-medium font-mono-numbers">16 frentes cadastradas</div>
      </div>

      <div className="space-y-4">
        {tree.map(t => (
          <div key={t.area} className="glass-panel p-4">
            <div className="flex items-center justify-between font-semibold text-sm text-[#081e2b] border-b border-slate-100 pb-2 mb-3">
              <span className="flex items-center gap-2"><span>📂</span> {t.area}</span>
              <span className="bg-slate-100 text-slate-600 text-[10px] px-2 py-0.5 rounded-full font-mono-numbers">{t.items.length} locais</span>
            </div>
